...
export const RegisterPage = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!name || !email || !targetExam || !password) return;
    dispatch(registerUserAsync({ name, email, mobile, targetExam, password }));
  };
    ...
    <main className="flex min-h-screen w-full flex-col lg:flex-row bg-background text-foreground">
      ...
      <section className="w-full lg:w-[45%] flex flex-col justify-center items-center p-6 md:p-12 bg-background relative min-h-screen">
        ...
        <div className="w-full max-w-[420px] bg-surface-container-lowest rounded-2xl shadow-xl border border-outline-variant p-8 md:p-10">
          <h2 className="text-2xl font-bold text-primary font-[Poppins] mb-1">Create Account</h2>
          <p className="text-xs text-on-surface-variant mb-6">Enter details below to register your profile</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-bold text-on-surface-variant uppercase tracking-wide">Full Name</label>
              <input
                type="text"
                value={name}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-bold text-on-surface-variant uppercase tracking-wide">Mobile Number</label>
              <div className="flex rounded-lg border border-outline-variant bg-transparent overflow-hidden focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
                <span className="flex items-center justify-center px-3 bg-surface-container-low text-on-surface-variant text-sm border-r border-outline-variant">+91</span>
                <input
                  type="tel"
                  value={mobile}
                  onChange={(e) => setMobile(e.target.value)}
                  pattern="[0-9]{10}"
                  className="w-full px-3 py-2 text-sm border-none focus:ring-0 outline-none bg-transparent text-on-surface"
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[12px] font-bold text-on-surface-variant uppercase tracking-wide">Target Exam</label>
              <div className="relative">
                <select
                  value={targetExam}
                  onChange={(e) => setTargetExam(e.target.value)}
                  className="w-full rounded-lg border border-outline-variant bg-transparent px-4 py-2.5 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none appearance-none cursor-pointer text-on-surface"
                  required
                >
                  <option value="" disabled>Select target exam</option>
                  <option value="SSC CGL">SSC CGL</option>
                  <option value="UPSC CSE">UPSC CSE</option>
                  <option value="Banking (SBI/IBPS)">Banking (SBI/IBPS)</option>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-secondary-container hover:opacity-90 text-on-secondary-container py-2.5 rounded-lg text-sm font-bold transition-all shadow-md mt-2 flex justify-center items-center gap-2 active:scale-[0.98] disabled:opacity-50"
            >
              {loading ? (
